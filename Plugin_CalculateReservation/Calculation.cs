...
namespace Plugin_CalculateReservation
{
    public class Calculation
    {
        ...
        public void Calculate()
        {
            ...
            {
                percent += d;
                totalDiscount += Math.Round((d * unitPrice / 100), MidpointRounding.AwayFromZero);
            }
            ...
            if (rsv.Contains("bsd_taxcode"))
            {
                ...
                taxAmount = Math.Round((unitPrice - landValue - totalDiscount + pkgSellingAmount) * taxPercent / 100, MidpointRounding.AwayFromZero);
                // test = (unitPrice - landValue - totalDiscount + pkgSellingAmount) * taxPercent / 100;
                //throw new InvalidPluginExecutionException("aa" + taxAmount + " --- "+ test);
            }
            //throw new Exception(taxAmount + "");

            decimal net = unitPrice - totalDiscount + pkgSellingAmount;
            decimal maintenanceFee = Math.Round(net * maintenancePercent / 100,0);
            if (rsv.Contains("bsd_projectid"))
            {
                Guid idpro = ((EntityReference)rsv["bsd_projectid"]).Id;
                ...
                if (idpro == check || idpro == check1 || idpro == check2)
                {
                    ...
                    decimal mainten = Math.Round(net * per / 100, 0);
                    maintenanceFee = mainten;
                    decimal taxnew = (net + mainten) * 10 / 100;
                    // throw new InvalidPluginExecutionException("amount " + taxnew);
                    taxAmount = taxnew;
                    //throw new InvalidPluginExecutionException("mainten " + mainten + " taxAmount "+ taxAmount);
                }
            }
           // throw new InvalidPluginExecutionException("nghĩa "+ maintenanceFee);
            //decimal maintenanceFee = net * maintenancePercent / 100;
            //decimal managementFee = net * managementPercent / 100;
            // throw new InvalidPluginExecutionException("maintenansceFee " + maintenanceFee);
        ...
        private void GetDiscount(EntityReference disRef, string selectedDiscount, out decimal amount,decimal unitprice, out List<decimal> percents)
        {
            ...
            foreach (string input in strArray)
            {
                ...
                else
                {
                    ...
                    if (num == 100000000)//percent
                    {
                        ...
                        decimal a = Math.Round(((decimal)pro["bsd_percentage"] * unitprice / 100), MidpointRounding.AwayFromZero);
                        str.Append("ITEM: " + a);
            ...
            //throw new InvalidPluginExecutionException("TEST: " + amount);
        }
