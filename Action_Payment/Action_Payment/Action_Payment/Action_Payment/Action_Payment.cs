// TriCM 16 08 20 payment workflow
// 16 08 20 - if tien khachhang tra du ra cho dot nay thi tao advance payment voi thong tin tuong ung - k cong so tien du do vao trong dot tiep theo
// 16 08 23 add condition: if pmsdtl la dot estimate handover - kiem tra neu pmsdtl ES có check
// 160917 - cong thuc tinh Interestcharge:
// lateday = duedate(installment) + graceday(interestMaster)
// if reciept date > lateday - tinh interestcharge tren so tien tra cua payment do.
// so sah interest moi duoc tinh voi  Interest Amount / Tolerance Interest Percentage -
// lay giá trị nhỏ hơn.
// 160926 them 2 field bsd_balance & bsd_differentamount

//@@ 161006
//$$ not use Outstanding more - all field input to Installment
//%% if payment for deposit - re-generate PMS DTL for Reservation

//@@ 161012 if deposit payment - if amount pay > deposit fee
// push so tien du vao 1st installment
// if amount pay > 1st amphase - create advance payment

// $$  161124 - bo phi quanly ( management fees ) ra khoi total amount
// $$ co them fan waiver trong installment
// 161213 - tinh interest charge: neu grace day = 15, duedate =1 , payment date = 10 , thi cho du interest charge type = grace hay duedate deu k tinh interest charge
// 161215 - !! total amount cua OE se la mau so tinh total percent
// !! tu so la total amount pay cho cac installment - cong them 2% maintenance fees = total percent = 102%

/// 170114
// chuyen payment tu workflow sang action

// 170314
// tam thoi k su dung field waiver amount cua installment nua - su dung waiver ( instsallment) ( vi co them field waiver installment & waiver interest charge )
// balance cua payment hien tai = amount phase - amount was paid - amount waiver ( installment) thay vi - waiver amount

//170325 - new function to re-generate duedate all installment of Quote - with input condition - auto date - not check isdeposited

// 170427
// new update
// transaction subgrid - luu vao 2 field ( luc truoc la 1 field)
// bsd_arraypsdid + bsd_arrayamountpay - for installment subgrid
// bsd_arrayinstallmentinterest + bsd_arrayinterestamount - for interest charge subgrid
...
/// 170506
// thanh toan fees -
// payment type = fees - cho phep thanh toan nhieu lan - neu thanh toan amount pay < total amount fees - uu tien thanh toan maintenance fees truoc
// cong tong tien thanh toan maintenance fees vao total amount paid cua OE.

/// 170512
// thanh toan tien fees dua vao truong hop thanh toan transaction pm khi diffe AM > 0
//170515 - neu op date trong unit cua payment null thi k cho thanh toan fees

/// 170608
// them bsd_paiddate - chua du lieu - khi payment cho INS - chuyen trang thai Paid thi update payment date vao field nay

/// 170807
// entity Miscellinous - chua du lieu cua INS - neu INS co MIS ( khi thanh toan payment se load du lieu MIS len sub-grid nhu fees.. cac INS chua paid)
// neu payment with difference amount > 0 - check if MIS complete  - update status reason to Paid else ...
// add new code to Transaction PM region

// neu payment type = Other thi check miscellaneous
// update ngay 23/7
//Thanh 
using System;
    ...
    public class Action_Payment : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                ...
                {
                    strMess.AppendLine("vào 111111111111111");
                    ...
                    if (pm_statuscode == 1)  // payment = active
                    {
                        ...
                        decimal d_inter = 0;
                        int i_lateday = 0;
                        Payment payment = new Payment(serviceProvider, strMess);
                    ...
                    ////parameter:
                    //req["EnCallFrom"] = paymentEn;
                    ////execute the request
                    //OrganizationResponse response = service.Execute(req);

                    //Invoice invoice = new Invoice(serviceProvider, strMess);
                    //Guid guiInsId = paymentEn.Contains("bsd_paymentschemedetail") ? PaymentDetailEn.Id: new Guid();
                    //invoice.processPayment(paymentEn, guiInsId);
                    strMess.AppendLine("23");
                        //-thành
                        #endregion
                ...
                }// end if target = payment
                 //throw new InvalidPluginExecutionException("aaaaaaaaaaaaaa");
                ...
                if (ec_ins.Entities[i].Contains("bsd_duedatecalculatingmethod"))
                {
                    ...
                    if (i_dueCalMethod == 100000001)
                    {
                        ...
                        if (payment_type == null || payment_type == 1)//default or month
                        {
                            ...
                            int type = (int)((OptionSetValue)ec_ins.Entities[i]["bsd_nextperiodtype"]).Value;
                            if (type == 1)//month
                            {
                                i_nextMonth = ec_ins.Entities[i].Contains("bsd_numberofnextmonth") ? (int)ec_ins.Entities[i]["bsd_numberofnextmonth"] : 0;
                                //throw new InvalidPluginExecutionException("Month=" + i_nextMonth.ToString());
                            ...
                            if (type == 2)//day
                            {
                                ...
                                //if (ec_ins.Entities[i].Contains("bsd_numberofnextdays"))
                                //    extraDay = ec_ins.Entities[i].GetAttributeValue<double?>("bsd_numberofnextdays").GetValueOrDefault();
                                //throw new InvalidPluginExecutionException("Day= " + dt_date.AddDays(ec_ins.Entities[i].GetAttributeValue<double?>("bsd_numberofnextdays").GetValueOrDefault()).ToShortDateString());

                                //dt_date = dt_date.AddDays(ec_ins.Entities[i].GetAttributeValue<double?>("bsd_numberofnextdays").GetValueOrDefault());
                                dt_date = dt_date.AddDays(extraDay);
                            }

                            //en_up.LogicalName = ec_pms.Entities[orderNumber].LogicalName;
                            //en_up.Id = tmpEn.Id;
                            //dbd10b9d-d7df-4609-a921-d5c9113dbc72
                            //Entity test = new Entity("bsd_paymentschemedetail", ec_pms.Entities[orderNumber].Id);
                            Entity en_up = new Entity("bsd_paymentschemedetail", ec_pms.Entities[orderNumber].Id);
        ...
        private EntityCollection get_InsList(IOrganizationService crmservices, Guid quoteID)
        {
            string fetchXml =
        ...
        //
        private EntityCollection get_Workflow(IOrganizationService crmservices)
        {
            string fetchXml =
              @"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                  <entity name='workflow'>
                    ...
                    <filter type='and'>
                      ...
                      <condition attribute='name' operator='eq' value='WF_PaymentScheme_Reservation' />
                    </filter>
                  </entity>
                </fetch>";
            EntityCollection entc = crmservices.RetrieveMultiple(new FetchExpression(fetchXml));
