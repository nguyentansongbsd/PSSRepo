...
function btnView_Generate() {
    debugger;
    try {
        window.top.$ui.Confirm('Confirm', 'Do you want to genarate Warning notices?', function (e) {
            window.top.ExecuteAction(
            ''
            , ''
            , 'bsd_Action_WarningNotices_GenerateWarningNotices'
            ...
            , function (result) {
                ...
                if (result != null && result.status != null) {
                    if (result.status == 'error')
                        ...
                        if (result.data.returnN.value >= 1) {
                            var grid = document.getElementById('crmGrid');
                            if (grid && grid.control)
                                grid.control.refresh();
                        }
                        window.top.$ui.Dialog('Message','Generated ' + result.data.returnN.value + ' record.');
