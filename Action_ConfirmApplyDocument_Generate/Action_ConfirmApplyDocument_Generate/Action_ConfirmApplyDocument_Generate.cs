...
namespace Action_ConfirmApplyDocument_Generate
{
    public class Action_ConfirmApplyDocument_Generate : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            else if (input01 == "Bước 02" && input02 != "" && input03 != "")
            {
                ...
                var reference = new EntityReference("bsd_applydocument", Guid.Parse(input03));
            ...
            else if (input01 == "Bước 03" && input02 != "" && input04 != "")
            {
                ...
                { "bsd_project", "bsd_applydocumenttype", "bsd_user", "bsd_receiptdate", "bsd_createddate" }));
                ...
                if (enConfirmPayment.Contains("bsd_receiptdate"))
                {
                    Receipt_Date = true;
                    bsd_paymentactualtime = DateTime.Parse(RetrieveLocalTimeFromUTCTime((DateTime)enConfirmPayment["bsd_receiptdate"]).ToString("MM/dd/yyyy"));
                }
                ...
                QueryExpression q = new QueryExpression("bsd_applydocument");
                q.ColumnSet = new ColumnSet(new string[] { "bsd_applydocumentid", "bsd_receiptdate", "createdon" });
                ...
                if (enConfirmPayment.Contains("bsd_applydocumenttype")) q.Criteria.AddCondition(new ConditionExpression("bsd_transactiontype", ConditionOperator.Equal, ((OptionSetValue)enConfirmPayment["bsd_applydocumenttype"]).Value));
                if (Receipt_Date) q.Criteria.AddCondition(new ConditionExpression("bsd_receiptdate", ConditionOperator.NotNull));
                ...
                {
                    bool check = false;
                    if (Receipt_Date && Created_Date && item.Contains("bsd_receiptdate") && item.Contains("createdon"))
                    {
                        DateTime paymentactualtime = DateTime.Parse(RetrieveLocalTimeFromUTCTime((DateTime)item["bsd_receiptdate"]).ToString("MM/dd/yyyy"));
                    ...
                    else if (Receipt_Date && item.Contains("bsd_receiptdate"))
                    {
                        DateTime paymentactualtime = DateTime.Parse(RetrieveLocalTimeFromUTCTime((DateTime)item["bsd_receiptdate"]).ToString("MM/dd/yyyy"));
