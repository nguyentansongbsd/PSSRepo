...
namespace Action_HandoverNotices_GenerateHandoverNotices
{
    public class Action_HandoverNotices_Generate : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            query.LinkEntities[0].LinkCriteria.AddCondition(new ConditionExpression("bsd_types", ConditionOperator.Equal, "100000002")); // Update for Handover Notices
            ...
            foreach (Entity detail in list.Entities)
            {
                ...
                if (detail.Contains("bsd_optionentry") && CheckExistHandoverNotices(service, (EntityReference)detail["bsd_optionentry"]) == false)
                {
                    ...
                    Entity UpEHDEn = service.Retrieve(((EntityReference)detail["bsd_updateestimatehandoverdate"]).LogicalName,
                        ((EntityReference)detail["bsd_updateestimatehandoverdate"]).Id,
                            new ColumnSet(new string[] { "bsd_name", "bsd_types", "bsd_project", "bsd_opdate", "bsd_simulationdate", "bsd_generated", "bsd_isincludelastinstallment" }));
                    ...
                    if (detail.Contains("bsd_optionentry"))
                    {
                        ...
                        EntityCollection calculateOther = CalculateOther(service, OE.ToEntityReference(), bsd_isincludelastinstallment);
        ...
        private bool CheckExistHandoverNotices(IOrganizationService crmservices, EntityReference oe)
        {
            string fetchXml =
                  @"<fetch version='1.0' output-format='xml-platform' count='1' mapping='logical' distinct='false'>
                      <entity name='bsd_handovernotice'>
                        ...
                        <filter type='and'>
                          ...
                          <condition attribute='statecode' operator='eq'  uitype='salesorder' value='0' />
                        </filter>
                      </entity>
                    </fetch>";
            fetchXml = string.Format(fetchXml, oe.Id);
