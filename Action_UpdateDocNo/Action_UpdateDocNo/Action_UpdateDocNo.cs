...
namespace Action_UpdateDocNo
{
    public class Action_UpdateDocNo : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            ...
            if (type == "Buoc01" && idRecord != "" && idUser != "" && input != "")
            {
                ...
                enTarget["bsd_documentno"] = input;
                service.Update(enTarget);
            }
            else if (type == "Buoc02" && idRecord != "" && idUser != "" && input != "")
            {
                traceService.Trace("Bước 02");
                service = factory.CreateOrganizationService(Guid.Parse(idUser));
                Entity enTarget = new Entity("bsd_updatedocno");
                enTarget.Id = Guid.Parse(idRecord);
