...
function quoVis_ConfirmDeposit() {
    ...
    if ((Xrm.Page.getAttribute("statuscode").getValue() == 100000000 || Xrm.Page.getAttribute("statuscode").getValue() == 100000006) && ((CheckRole(Xrm.Page.context.getUserRoles(), ["CLVN_FIN_Accountant"])) || (CheckRole(Xrm.Page.context.getUserRoles(), ["CLVN_FIN_Chief Accountant"])) || (CheckRole(Xrm.Page.context.getUserRoles(), ["CLVN_FIN_Financal Controller"]))
        || CheckRole(Xrm.Page.context.getUserRoles(), ["CLVN_FIN_Finance Manager"])
        || CheckRole(Xrm.Page.context.getUserRoles(), ["CLVN_S&M_Senior Sale Staff"])
        || CheckRole(Xrm.Page.context.getUserRoles(), ["CLVN_S&M_Sales Manager"])
        || heckRole(Xrm.Page.context.getUserRoles(), ["CLVN_S&M_Head of Sale"]))) {
        return true;
    }
    return false;
}
